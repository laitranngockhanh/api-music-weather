...
class SongController extends Controller
{
    ...
    public function store(Request $request)
    {
        ...
        if ($file->getSize() > 20480000) {
            return response()->json(['message' => 'File quá lớn! Giới hạn 20MB.'], 422);
        }
        ...
        try {
            ...
            $fileContent = stream_get_contents($fileStream, -1, 0);
            fclose($fileStream);
            $time = microtime(true) - $start;
            Log::info('Time to read file with stream', ['time' => $time]);
